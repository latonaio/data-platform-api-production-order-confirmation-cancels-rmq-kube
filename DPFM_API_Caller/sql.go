...
import (
	dpfm_api_input_reader "data-platform-api-production-order-confirmation-cancels-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-production-order-confirmation-cancels-rmq-kube/DPFM_API_Output_Formatter"
...
func (c *DPFMAPICaller) ItemsRead(
	...
	where := fmt.Sprintf("WHERE item.ProductionOrderConfirmationItem IS NOT NULL\nAND header.ProductionOrderConfirmation = %d", input.Header.ProductionOrderConfirmation)
	rows, err := c.db.Query(
		`SELECT 
			item.ProductionOrderConfirmation, item.ProductionOrderConfirmationItem
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_data as item
		INNER JOIN DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_header_data as header
